...
use axum::Json;
use cruise_storage::{CatalogEntry, HistoryPoint, PriceRepo};
use serde::{Deserialize, Serialize};
use std::sync::Arc;

use crate::auth::{AuthedUser, TgAuthOnly};
use crate::state::AppState;
use cruise_api::{CruiseClient, CruiseClientConfig};
use cruise_storage::NewUser;
...
pub async fn register(
    ...
    let report = cruise_core::discover_and_persist_bookings(
        &email,
        &body.password,
...
pub async fn refresh_catalog(
    ...
    let persisted = cruise_core::refresh_catalog_for_booking(&client, s.repo.as_ref(), &booking)
        .await
