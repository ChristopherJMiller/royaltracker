...
//! Usage: `cargo run -p cruise-telegram --example test-alert`

use cruise_telegram::{bot, send_diff, DiffContext};
use cruise_types::Diff;
...
async fn main() -> anyhow::Result<()> {
    ...
    // Prefer admin_chat_id; fall back to $CRUISE_TEST_CHAT_ID for ad-hoc runs.
    let chat_id = toml["telegram"]["admin_chat_id"]
        .as_integer()
        .filter(|v| *v != 0)
        .or_else(|| std::env::var("CRUISE_TEST_CHAT_ID").ok().and_then(|s| s.parse().ok()))
        .ok_or_else(|| anyhow::anyhow!(
            "set [telegram].admin_chat_id in config.toml, or run with CRUISE_TEST_CHAT_ID=<id>"
        ))?;
