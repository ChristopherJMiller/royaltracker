...
use cruise_api::{BookingSummary, CruiseClient, CruiseClientConfig};
...
use cruise_storage::{connect, DefaultRepo, PriceRepo};
use cruise_telegram::bot;
use cruise_types::User;
...
async fn main() -> Result<()> {
    ...
    for user in users {
            ...
            scrape_one_user(&repo, &cipher, &bot, &cfg.rcg_basic_auth_b64, &user).await
        {
            ...
            let _ = cruise_telegram::send_text(
                &bot,
