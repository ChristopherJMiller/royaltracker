...
use cruise_config::Config;
use cruise_crypto::Cipher;
use cruise_storage::{connect, DefaultRepo, NewUser, PriceRepo};
...
async fn main() -> Result<()> {
    ...
    let web_state = cruise_web::AppState {
        repo: repo.clone(),
        cipher: cipher.clone(),
    ...
    let web_handle = tokio::spawn(async move {
        if let Err(e) = axum::serve(listener, cruise_web::router(web_state)).await {
            tracing::error!(error = %e, "web server crashed");
        }
    });

    tracing::info!("cruise-bot starting; long-poll mode");
